...
CREATE TRIGGER insert_sales_items AFTER UPDATE ON ospos_sales_items FOR EACH ROW
BEGIN
SET @json = '{';
SET @json = CONCAT(@json, '"sale_id"', ":",'"', NEW.sale_id,'"',',');
SET @json = CONCAT(@json, '"item_id"', ":",'"', NEW.item_id,'"',',');
...
CREATE TRIGGER insert_sales_items AFTER DELETE ON ospos_sales_items FOR EACH ROW
BEGIN
SET @json = '{';
